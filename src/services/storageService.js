    ...
    .factory('storageService', ['$http', function ($http) {
      ...
      function normalizeTransactionData (transactionData) {
        return angular.extend(
          {},
          emptyTransaction,
          { merchantLogo: getMerchantLogo(transactionData.merchant) },
          transactionData
        );
      }
      function getMerchantLogo (merchantName) {
        var merchantIndex = state.merchants.findIndex(function (merchant) {
          return (merchant.name.toLowerCase() === merchantName.toLowerCase());
        });

        return (merchantIndex > -1)
          ? state.merchants[merchantIndex].logo
      ...
      function updateBalance (transactionValue) {
        state.balance -= transactionValue;
      }
